...
import { editMeal, removeMeal } from "../redux/cart/cartSlice";
      ...
      <div className="w-full p-2">
        {itemsInCart?.map((meal) => {
          return (
            ...
            >
              <span className="flex justify-end text-error hover:cursor-pointer" onClick={()=>{dispatch(removeMeal(meal[0].id))}}>
                <RiDeleteBinLine />
              </span>
              <div className="flex flex-row pt-1">
                <img src={meal[2]} alt="photo" className="rounded w-16" />
                <div className="flex flex-col">
                  <p className="ml-12 flex items-center w-full justify-between">
                    {meal[0].name}
                  </p>
                  <div className="flex justify-start ml-12 text-xl font-medium">
                    <span
                      ...
                      onClick={() => {
                        dispatch(
                          editMeal({ id: meal[0].id, sign: "decrease" })
                        );
                      }}
                    >
                      -
                    ...
                    <span
                      ...
                      onClick={() => {
                        dispatch(
                          editMeal({ id: meal[0].id, sign: "increase" })
                        );
                      }}
                    >
                      +
                    </span>
                    <span className="p-2 ml-24 pt-2">
                      {meal[1] * meal[0].price}€
                    </span>
                  </div>
                </div>
